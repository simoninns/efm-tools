...
void Audio::set_data(const QVector<int16_t>& data) {
    if (data.size() != 12) {
        qFatal("Audio::set_data(): Data size of %d does not match frame size of %d", data.size(), 12);
    }
    audio_data = data;
}
...
void Audio::set_error_data(const QVector<int16_t>& error_data) {
    if (error_data.size() != 12) {
        qFatal("Audio::set_error_data(): Error sata size of %d does not match frame size of %d", error_data.size(), 12);
    }
