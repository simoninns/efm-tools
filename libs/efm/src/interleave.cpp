...
QVector<bool> Interleave::interleave(const QVector<bool> &inputData)
{
    // Ensure input data is 24 bytes long
    if (inputData.size() != 24) {
        qDebug() << "Interleave::interleave - Input data must be 24 values long";
        return inputData;
    }

    // Interleave the input data
    QVector<bool> outputData(24);
...
QVector<bool> Interleave::deinterleave(const QVector<bool> &inputData)
{
    // Ensure input data is 24 bytes long
    if (inputData.size() != 24) {
        qDebug() << "Interleave::deinterleave - Input data must be 24 values long";
        return inputData;
    }
